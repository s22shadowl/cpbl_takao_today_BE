...
def scrape_and_store_season_stats():
    ...
    if not club_no:
        logger.error(f"在設定中找不到球隊 [{settings.TARGET_TEAM_NAME}] 的代碼 (ClubNo)。")
        return
            ...
            try:
                ...
                for i, inning_li in enumerate(inning_buttons):
                    ...
                    if half_inning_section.count() > 0:
                        ...
                        logger.info(f"處理第 {inning_num} 局 [{settings.TARGET_TEAM_NAME}]，找到 {len(expand_buttons)} 個打席，準備展開...")
                ...
                db.commit()
                logger.info(f"成功提交比賽 {game_info.get('cpbl_game_id')} 的所有資料到資料庫。")
...
def scrape_entire_month(month_str=None):
    ...
    logger.info(f"--- 開始執行 [逐月模式]，目標月份: {target_date_obj.strftime('%Y-%m')} ---")
    
    if target_date_obj.year > today.year or (target_date_obj.year == today.year and target_date_obj.month > today.month):
        logger.warning(f"目標月份 {target_date_obj.strftime('%Y-%m')} 是未來月份，任務中止。")
        return
...
def scrape_entire_year(year_str=None):
    ...
    for month in range(start_month, end_month + 1):
        ...
        if html_content:
            ...
            logger.info(f"月份 {year_to_scrape}-{month:02d} 共解析到 {len(all_month_games)} 場比賽。")
            if all_month_games:
                games_to_process = [game for game in all_month_games if datetime.datetime.strptime(game['game_date'], "%Y-%m-%d").date() <= today]
                _process_filtered_games(games_to_process)
