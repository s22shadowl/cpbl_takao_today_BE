...
def task_update_schedule_and_reschedule():
    ...
    try:
        ...
        logger.error(f"Dramatiq Worker 在執行賽程更新任務時發生嚴重錯誤: {e}", exc_info=True)
...
def task_scrape_single_day(date_str: Optional[str] = None):
    ...
    try:
        ...
        logger.info(f"--- Dramatiq Worker: 單日爬蟲任務 for {date_str or '今天'} 執行完畢 ---")
    except Exception as e:
        logger.error(f"Dramatiq Worker 在執行單日爬蟲任務時發生嚴重錯誤: {e}", exc_info=True)
...
def task_scrape_entire_month(month_str: Optional[str] = None):
    ...
    try:
        ...
        logger.info(f"--- Dramatiq Worker: 逐月爬蟲任務 for {month_str or '本月'} 執行完畢 ---")
    except Exception as e:
        logger.error(f"Dramatiq Worker 在執行逐月爬蟲任務時發生嚴重錯誤: {e}", exc_info=True)
...
def task_scrape_entire_year(year_str: Optional[str] = None):
    ...
    try:
        ...
        logger.info(f"--- Dramatiq Worker: 逐年爬蟲任務 for {year_str or '今年'} 執行完畢 ---")
    except Exception as e:
        logger.error(f"Dramatiq Worker 在執行逐年爬蟲任務時發生嚴重錯誤: {e}", exc_info=True)