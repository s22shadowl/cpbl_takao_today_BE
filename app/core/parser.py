...
from app.config import TARGET_TEAM_NAME, TARGET_PLAYER_NAMES

def parse_schedule_page(html_content, year):
    """
    從賽程頁面的 HTML 中，解析出該月份「所有」的比賽基本資訊。

    :param html_content: 賽程頁面的完整 HTML 字串
    :param year: 當前處理的年份 (int)
    :return: 一個包含多場比賽資訊字典的列表 (list of dicts)
    """
    if not html_content:
    ...
    if not schedule_table:
        logging.warning("在賽程頁 HTML 中找不到賽程表格。")
        return []
    ...
    for row in all_rows:
        ...
        if date_cell:
            ...
            try:
                ...
                logging.warning(f"無法解析日期文字: '{date_text}'，跳過此日期。")
                continue
        ...
        try:
            ...
            if remark_cell:
                ...
                if note_tag: game_data['status'] = note_tag.text.strip()
                elif game_data.get('home_score') is not None: game_data['status'] = '已完成'
                else: game_data['status'] = '未開始'
...
def parse_box_score_page(html_content):
    """從 Box Score 頁面 HTML 中，解析出目標球隊的目標球員數據。"""
    ...
    for block in team_stat_blocks:
        ...
        if current_team_name != TARGET_TEAM_NAME: continue
        ...
        for player_row in player_summary_rows:
            ...
            if player_name not in TARGET_PLAYER_NAMES: continue
            ...
            try:
                ...
                parsed_player_data.append(player_full_data)

            except Exception as e:
                logging.error(f"解析球員 [{player_name}] 的 Box Score 數據時發生錯誤: {e}", exc_info=True)
...
def parse_season_stats_page(html_content):
    ...
    for row in player_data_rows:
        ...
        if player_name in TARGET_PLAYER_NAMES:
            try:
                stats_data = { "player_name": player_name, "team_name": TARGET_TEAM_NAME }
