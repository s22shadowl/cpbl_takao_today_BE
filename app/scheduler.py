...
def _schedule_daily_scraper(game_date: str, game_time: str, matchup: str):
    ...
    if not game_time or not isinstance(game_time, str) or ":" not in game_time:
        logger.info(
            f"比賽 on {game_date} ({matchup}) 時間未定，將預設排程設定於 22:00。"
        )
        game_time = "18:35"
    ...
    try:
        ...
        logger.error(
            f"解析時間字串時發生錯誤 for game {game_date} ('{game_datetime_str}'): {ve}"
        )
...
def setup_scheduler(scrape_all_season: bool = False):
    ...
    if jobs:
        try:
            ...
            logger.info("--- 下一次排程任務 ---")
            logger.info(
                f"執行時間: {next_job.next_run_time.strftime('%Y-%m-%d %H:%M:%S %Z')}"
            )
