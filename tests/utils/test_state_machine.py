...
@pytest.mark.parametrize(
    ...
    [
        # --- 基本安打與保送 ---
        ([None, None, None], '打者A', "擊出右外野滾地球，一壘安打 。", ['打者A', None, None]),
        ([None, None, None], '打者A', "擊出左外野平飛球，二壘安打 。", [None, '打者A', None]),
        ([None, None, None], '打者A', "擊出中外野深遠飛球，三壘安打 。", [None, None, '打者A']),
        ([None, None, None], '打者A', "獲得一個四壞球保送", ['打者A', None, None]),

        # --- 跑者推進 & 打者上壘 ---
        (['跑者B', None, None], '打者A', "擊出一壘安打，一壘跑者跑者B上二壘。", ['打者A', '跑者B', None]),
        # --- 僅有「安打」不算是正規關鍵字 ---
        (['跑者B', '跑者C', None], '打者A', "擊出安打。二壘跑者跑者C 上三壘。一壘跑者跑者B 上二壘。", [None, '跑者B', '跑者C']),
        
        # --- 【新增】失誤優先級測試 ---
        # 只有失誤
        ([None, None, None], '打者A', "因投手傳球失誤上一壘", ['打者A', None, None]),
        # 同時有安打和失誤，應以失誤結果為準
        (['跑者B', None, None], '打者A', "擊出一壘安打，但因外野手失誤，打者上二壘；一壘跑者跑者B上三壘。", ['打者A', None, '跑者B']),
    ],
    ids=[
        # --- 原有案例 ---
...
def test_update_runners_state(runners_before, hitter, description, expected_runners_after):
    """測試根據文字描述更新跑者狀態的各種情境。"""
    assert _update_runners_state(runners_before, hitter, description) == expected_runners_after