...
@patch('app.scheduler.datetime')
def test_schedule_daily_scraper_skips_past_run_date(mock_datetime, mock_scheduler_and_deps):
...
@patch('app.scheduler.datetime')
def test_setup_scheduler_default_behavior(mock_datetime, mock_schedule_func, mock_scheduler_and_deps):
    """測試 setup_scheduler 預設行為 (只排程今天及未來的比賽)。"""
    mock_db_actions = mock_scheduler_and_deps["db_actions"]
    mock_scheduler = mock_scheduler_and_deps["scheduler"]
    
    mock_datetime.now.return_value = datetime(2025, 7, 1, 10, 0, 0, tzinfo=mock_scheduler.timezone)
    
    fake_schedules = [
        models.GameSchedule(game_date=date(2025, 6, 30), game_time='18:35', matchup='過去的比賽'),
        models.GameSchedule(game_date=date(2025, 7, 1), game_time='18:35', matchup='今天的比賽'),
        models.GameSchedule(game_date=date(2025, 7, 2), game_time='18:35', matchup='未來的比賽'),
    ]
